  ...
  return (
    <div className="flex flex-col gap-6 pb-6">
      ...
      <div className="flex flex-wrap gap-2">
        ...
        <BrandHubStatChip label="Video" value={videoCount} />
        <BrandHubStatChip
          label="Format scrape"
          value={scrapeMediaTypeLabel(data.mediaType)}
        />
        <BrandHubStatChip label="Aktif" value={activeCount} tone="success" />
        {data.batchStatus ? (
          <BrandHubStatChip
            label="Status"
      ...
      {data.errorMessage && data.batchStatus === "FAILED" ? (
        <div className={cn(hub.panel, "border-destructive/30 bg-destructive/5 p-4 text-sm")}>
          {data.errorMessage}
        </div>
      ) : null}

      {hasKeywordSearch ? (
        <div className={cn(hub.panel, "text-muted-foreground p-4 text-sm leading-relaxed")}>
      ...
      {(data.aiSummary || data.aiInsights) && data.ads.length > 0 ? (
        ...
        >
          <div className={cn(hub.panel, "flex flex-col gap-4 p-4")}>
            {data.aiSummary ? (
      ...
      >
        {filteredAds.length === 0 ? (
          <div className={cn(hub.panel, "text-muted-foreground p-8 text-center text-sm")}>
            {data.ads.length === 0
