...
import {
  createBrandSocialListeningMonitor,
  deleteBrandSocialListeningMonitor,
  refreshBrandSocialListeningMonitor,
  toggleBrandSocialListeningMonitorActive,
} from "@/actions/brand-social-listening";
...
export function BrandSocialListeningClient({
  ...
  function handleCreate() {
    ...
    startTransition(async () => {
      try {
        const result = await createBrandSocialListeningMonitor({
          name,
          keywords,
          platforms,
        });
        ...
        setName("");
        setKeywordsText("");
        router.push(`/brand-hub/social-listening/${result.id}`);
      } catch (err) {
  ...
  function handleRefresh(id: string) {
    startTransition(async () => {
      try {
        await refreshBrandSocialListeningMonitor(id);
  ...
  function handleDelete(id: string) {
    startTransition(async () => {
      try {
        await deleteBrandSocialListeningMonitor(id);
  ...
  function handleToggleActive(id: string, isActive: boolean) {
    startTransition(async () => {
      try {
        await toggleBrandSocialListeningMonitorActive(id, isActive);
        router.refresh();
      } catch (err) {
