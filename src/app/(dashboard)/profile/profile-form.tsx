...
async function copyProfileUrl(profileSharePath: string) {
  try {
    const url =
...
/* ========================================================================== */
/*  Bagian: Profil (foto, nama, bio) — identitas publik                       */
/* ========================================================================== */
  ...
  async function onSaveBasics() {
    startBasics(async () => {
      try {
        ...
        await update({
          user: { name: name.trim() || null, bio: bio.trim() || null },
        });
    ...
    <div className="flex w-full flex-col gap-6">
      ...
      <Card className="border-border/60 shadow-none">
        <CardHeader>
          ...
          <CardDescription>
            Nama & deskripsi yang tampil di Kanban, chat, dan profil publik.
          </CardDescription>
        </CardHeader>
        <CardContent className="grid gap-4">
          ...
          <div className="flex justify-end">
            ...
            >
              {pendingBasics ? "Menyimpan…" : "Simpan profil"}
            </Button>
          </div>
        </CardContent>
      </Card>
...
/* ========================================================================== */
/*  Bagian: Akun & keamanan (email, WhatsApp, kata sandi)                     */
/* ========================================================================== */
...
export function AccountSecuritySection({
  ...
  function onSaveContact() {
    startContact(async () => {
      try {
        await updateProfileContact({ whatsappPhone: whatsappPhone.trim() });
        toast.success("Kontak disimpan.");
        router.refresh();
      } catch (e) {
        toast.error(actionErrorMessage(e, "Gagal menyimpan kontak."));
      }
    });
  }
    ...
    <div className="flex w-full flex-col gap-6">
      ...
      <Card className="border-border/60 shadow-none">
        <CardHeader>
          ...
          <CardDescription>
            Email login tidak bisa diubah. WhatsApp dipakai untuk notifikasi
            tugas.
          </CardDescription>
        </CardHeader>
        <CardContent className="grid gap-4">
          ...
          <div className="flex justify-end">
            ...
            >
              {pendingContact ? "Menyimpan…" : "Simpan kontak"}
            </Button>
          </div>
        </CardContent>
      </Card>
...
/* ========================================================================== */
/*  Bagian: Studio tampilan profil (LEGACY) — fallback bila gamifikasi mati   */
/* ========================================================================== */
...
export function AppearanceStudio({
  ...
  // Sinkronkan ke data server terbaru bila props initial berubah (mis. setelah
  // simpan memicu router.refresh). Pola "adjust state during render" — bukan
  // effect — jadi tidak memicu render bertingkat. https://react.dev/learn/you-might-not-need-an-effect
  const initialSig = `${initialBannerPreset}|${initialBannerPattern}|${initialTagline}|${initialAccentHex ?? ""}|${initialSticker ?? ""}|${initialAvatarFrame}`;
  const [syncedSig, setSyncedSig] = useState(initialSig);
  if (syncedSig !== initialSig) {
  ...
  return (
    <Card
      className="border-border/70 overflow-hidden ring-1 ring-black/[0.03] dark:ring-white/[0.04]"
    ...
    >
      ...
      <CardContent className="grid gap-6">
        <PreviewBoard
          displayName={initialName.trim() || email.split("@")[0]!}
          email={email}
          tagline={tagline}
        ...
        <div className="flex flex-wrap items-center gap-2 text-xs">
          <Button type="button" variant="outline" size="sm" onClick={() => void copyProfileUrl(profileSharePath)}>
            <Link2 className="size-4" />
            Salin tautan profil
          </Button>
