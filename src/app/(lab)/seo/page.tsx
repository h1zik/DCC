...
export default async function SeoOverviewPage() {
  ...
  return (
    <LabPageShell>
      ...
      />
      <LabSection title="Metrik" description="Kondisi SEO terkini.">
        <div className="flex flex-wrap gap-3">
          ...
          <LabStatChip
            ...
            tone="success"
          />
          <LabStatChip label="Total keyword" value={keywordCount} />
          <LabStatChip label="Keyword dilacak" value={trackedCount} />
          <LabStatChip
            label="Site health"
            value={latestHealthScore != null ? `${latestHealthScore}/100` : "—"}
          />
          <LabStatChip label="Isu teknis" value={technicalIssues} tone="warning" />
          <LabStatChip
      ...
      {gsc?.configured ? (
        <div className={cn(lab.card, "p-4")}>
          <div className="flex flex-wrap items-center gap-x-6 gap-y-2">
            <div>
              <p className={cn(lab.label)}>Klik organik 28 hari (GSC)</p>
            ...
            <div>
              <p className={cn(lab.label)}>Impresi 28 hari</p>
              <p className="text-2xl font-bold tabular-nums">
            ...
            {gsc.topQueries.length > 0 ? (
              <div className="min-w-0 flex-1">
                <p className={cn(lab.label, "mb-1")}>Query teratas</p>
                <div className="flex flex-wrap gap-1.5">
                  {gsc.topQueries.map((q) => (
                    <span
                      key={q.key}
                      className={cn(lab.nestedPanel, "px-2 py-0.5 text-xs")}
                    >
                      {q.key}{" "}
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        {/* Pipeline konten */}
        <div className={cn(lab.card, "p-4")}>
          <p className={cn(lab.label, "mb-2")}>Pipeline konten</p>
          <div className="grid grid-cols-4 gap-2 text-center">
            {(
              ...
              <Link key={label} href={href} className={cn(lab.nestedPanel, "py-2")}>
        ...
        {/* Top movers */}
        <div className={cn(lab.card, "p-4")}>
          <p className={cn(lab.label, "mb-2")}>Top movers (vs cek sebelumnya)</p>
        ...
        {/* Biaya API bulan ini */}
        <div className={cn(lab.card, "p-4")}>
          <p className={cn(lab.label, "mb-1")}>Biaya DataForSEO bulan ini</p>
          <p className="text-2xl font-bold tabular-nums">
      ...
      <div className="grid gap-5 lg:grid-cols-[1fr_320px]">
        <LabSection title="Tren ranking" description="Posisi rata-rata 30 hari terakhir.">
        ...
        <LabSection title="Audit terbaru" description="Skor On-Page terakhir.">
            ...
            <div className="flex flex-col gap-2">
              {recentAudits.map((a) => (
                <Link
                  ...
                  className={cn(lab.card, "flex items-center gap-3 p-2.5")}
                >
                  <span
                    className={cn(
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {features.map((f) => (
            <Link
              key={f.href}
              href={f.href}
              className={cn(lab.card, lab.cardHover, "block p-5")}
            >
