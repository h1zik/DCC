  ...
  return (
    <div className="flex flex-col gap-6 pb-6">
      ...
      <div className="flex flex-wrap gap-2">
        <LabStatChip label="Winning score" value={data.winningScore} tone="accent" />
        <LabStatChip
          label="Tayang"
          value={data.daysRunning != null ? `${data.daysRunning} hari` : "—"}
        />
        <LabStatChip label="Varian kreatif" value={num(data.collationCount)} />
        <LabStatChip
          label="Status"
          value={data.isActive ? "Aktif" : "Berhenti"}
          tone={data.isActive ? "success" : "neutral"}
        />
        <LabStatChip label="Platform" value={data.platforms.length} />
      </div>
