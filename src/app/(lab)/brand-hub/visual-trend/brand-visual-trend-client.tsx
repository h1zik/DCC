...
import { LabEmptyState, LabSection, lab } from "@/components/lab/lab-primitives";
...
export function BrandVisualTrendClient({
  ...
  return (
    <div className={cn("grid gap-6 lg:grid-cols-[1fr_minmax(240px,300px)]", lab.entrance)}>
      <div className="flex flex-col gap-4">
        ...
        >
          {generatingAll ? (
            <div className={cn(lab.nestedPanel, "text-muted-foreground flex items-center gap-2 text-sm")}>
          ...
          {allBrief ? (
            <div className={cn(lab.panel, "border-primary/25")}>
              <BriefOutputPanel
                entry={allBrief}
                innerRef={(el) => {
        ...
        {collections.map((c, index) => {
          ...
          return (
            <div
              key={c.id}
              className={cn(lab.panel, lab.entrance, "space-y-4")}
              style={
                index > 0 && index < 8
      ...
      <aside className="flex flex-col gap-4">
        <LabSection title="Konteks pasar (Research)" delayMs={0}>
            ...
            {marketContext.length === 0 ? (
              ...
              <ul className="mt-3 space-y-2">
                {marketContext.map((item) => (
                  <li key={item.id} className={cn(lab.nestedPanel, "text-xs")}>
        ...
        <LabSection title="Visual Library" delayMs={50}>
          <div className={lab.panel}>
