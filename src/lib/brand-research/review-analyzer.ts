...
async function saveReviewAnalysis(
  ...
  await prisma.brandReviewAnalysis.upsert({
    where: { reviewId },
...
export async function analyzeBrandReviewSource(sourceId: string): Promise<void> {
  ...
  const analyzed = await prisma.brandReviewItem.findMany({
    where: { sourceId },
  ...
  await prisma.brandReviewSummary.upsert({
    where: { sourceId },
    create: {
      sourceId,
      positivePct: agg.positivePct,
  ...
  await syncModuleRecommendations({
    module: "brand-review-intelligence",
    sourceId,
    sourceLabel: `${source.competitorBrand} · ${source.productName}`,
    href: `/brand-hub/review-intelligence/${sourceId}`,
    plan: actionPlan,
  });
