 ...
 */
export async function recoverMisclassifiedBrandReviewScrapeJobs(
  limit = 5,
): Promise<number> {
  ...
  for (const job of jobs) {
    ...
    try {
      ...
      try {
        await pollBrandReviewScrapeJob(job.id);
        recovered += 1;
      } finally {
...
export async function executeBrandReviewScrapeJob(jobId: string): Promise<void> {
  ...
  try {
    ...
    if (job.apifyRunId) {
      await pollBrandReviewScrapeJob(jobId);
      return;
    }
    ...
    if (platformKey === "shopee" && isScraperApiConfigured()) {
      try {
        ...
        if (result.reviews.length > 0) {
          await completeBrandReviewScrapeFromNormalized(
            source.id,
            result.reviews,
            result.meta,
          );
    ...
    const latest = await prisma.brandResearchScrapeJob.findUnique({
      where: { id: jobId },
      select: { status: true },
    });
...
export async function pollBrandReviewScrapeJobsLight(): Promise<number> {
  const jobs = await prisma.brandResearchScrapeJob.findMany({
    where: {
      type: ResearchScrapeJobType.REVIEW_SCRAPE,
      status: ResearchScrapeJobStatus.RUNNING,
  ...
  for (const job of jobs) {
    ...
    try {
      ...
      console.error("[pollBrandReviewScrapeJobsLight]", job.id, err);
    } finally {
      activeReviewScrapeJobIds.delete(job.id);
    }
  }
...
export async function resumeStuckBrandReviewScrapeJobs(limit = 5): Promise<number> {
  await recoverMisclassifiedBrandReviewScrapeJobs(limit);
