...
export async function gatherBrandUspContext(input: {
  category: string;
  ...
  if (contextModules.reviewIntel) {
    ...
    let sources = explicitIds.length
      ? await prisma.brandReviewSource.findMany({
    ...
    if (sources.length === 0) {
      const all = await prisma.brandReviewSource.findMany({
        where: { status: "READY" },
        include: { summary: true },
  ...
  if (contextModules.competitor) {
    ...
    let picks = explicitIds.length
      ? await prisma.brandCompetitor.findMany({
    ...
    if (picks.length === 0) {
      const all = await prisma.brandCompetitor.findMany({
        where: { isActive: true },
        include: {
          skus: { orderBy: { lastSeenAt: "desc" }, take: 30 },
        },
  ...
  if (contextModules.trendRadar) {
    const digest = contextModules.trendDigestId
      ? await prisma.brandTrendDigest.findFirst({
          where: {
    ...
    if (digest) {
      const label = digest.isGlobal
        ? "Digest global"
        : (digest.ownerBrand?.name ?? "Per brand");
      resolvedSources.trendRadar = {
        ...
        meta: digest.generatedAt
          ...
          : undefined,
        href: `/brand-hub/trend-radar/${digest.id}`,
      };
  ...
  if (contextModules.keywordIntel) {
    const query = contextModules.keywordQueryId
      ? await prisma.brandKeywordQuery.findFirst({
          where: {
    ...
    if (query?.result) {
      resolvedSources.keywordIntel = {
        ...
        href: `/brand-hub/keyword-intel/${query.id}`,
      };
  ...
  if (contextModules.socialListening) {
    ...
    const batch = monitorId
      ? await prisma.brandSocialBatch.findFirst({
    ...
    if (batch?.summary) {
      ...
      if (useBatch) {
        resolvedSources.socialListening = {
          ...
          href: `/brand-hub/social-listening/${batch.monitorId}`,
        };
