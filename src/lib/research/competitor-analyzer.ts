...
export async function analyzeCompetitor(competitorId: string): Promise<void> {
  ...
  const extra = await computeCompetitorExtraMetrics(
    competitorId,
    competitor.category,
  );
...
Balas HANYA JSON valid:
{
  ...
  await syncModuleRecommendations({
    ...
    href: `/research-hub/competitor-tracker/${competitorId}`,
    plan: actionPlan,
  });
}
