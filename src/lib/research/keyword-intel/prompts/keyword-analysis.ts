...
import type { NormalizedKeywordSignal } from "@/lib/research/keyword-intel/keyword-signal-types";import { buildActionPlanInstruction } from "@/lib/research/prescriptive/prompt";
...
export function buildKeywordAnalysisPrompt(input: {
  ...
  gapCandidates: GapKeywordRow[];  forbiddenBrands?: string[];
}): string {
  const payload = input.signals.slice(0, 60).map((s) => ({
    ...
    trend: s.trend ?? null,
    source: s.source,
    listingSampleCount: s.listingSampleCount ?? null,
  }));
...
Tugas (NARRATIVE ONLY — jangan tambah keyword atau angka volume baru):
...
5. seasonalCalendar — anotasi event ID per bulan (Ramadan, 11.11, Lebaran, musim hujan) dengan keyword generik kategori — BUKAN prediksi volume
6. clusters — [{ name, keywords }] kelompokkan by intent/tema dari data mentah saja
