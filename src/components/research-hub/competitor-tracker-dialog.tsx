...
import { listCompetitorProductCategories } from "@/actions/research-competitor-product";
...
/** Hasil aksi "tambah ke tracker" — dipakai untuk navigasi toast "Lihat". */
export type CompetitorTrackerAddResult = { trackId: string; categoryId: string };

/**
 * Dialog generik "Tambah ke Competitor Tracker".
 * Logika sumber produk diserahkan ke `onAdd` agar bisa dipakai dari Product
 * Discovery maupun dari SKU competitor shop. `onAdd` menerima pilihan kategori
 * dan mengembalikan id track + kategori untuk navigasi.
 */
export function CompetitorTrackerDialog({
  productName,
  defaultCategoryName,
  onAdd,
  disabled,
  ...
  }) => Promise<CompetitorTrackerAddResult>;
  disabled?: boolean;
  className?: string;
}) {
      ...
      <DialogContent className="sm:max-w-md">
        ...
        <div className="space-y-4">
          ...
          {useNewCategory || categories.length === 0 ? (
            <div className="space-y-2">
              <Label htmlFor={inputId}>Nama kategori baru</Label>
              <Input
                id={inputId}
