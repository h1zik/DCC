...
function sentimentChipClass(value: SentimentFilterValue, active: boolean): string {
  ...
  switch (value) {
    ...
    default:
      return "bg-[color-mix(in_srgb,var(--lab-accent,var(--primary))_12%,transparent)] text-[var(--lab-accent,var(--primary))]";
  }
}
...
export function ReviewRawDataPanel({
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/set-state-in-effect -- fetch halaman review dari server action
    void load();
  }, [load, reviewCount]);
  ...
  if (reviewCount <= 0) {
    ...
    return (
      <div className={cn(lab.card, "p-4")}>
        <p className="bento-label mb-2">Raw Reviews</p>
        {empty}
      </div>
    );
  }
  ...
  return (
    <div className={cn(lab.card, "p-0")}>
      {/* Toolbar: judul + count di kiri, cari + export di kanan */}
      <div className="flex flex-col gap-3 p-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-muted-foreground text-xs">
            {hasFilter
              ? `${total.toLocaleString("id-ID")} dari ${reviewCount.toLocaleString("id-ID")} review`
        ...
        {SENTIMENT_FILTERS.map((f) => {
          ...
          return (
            <button
              ...
              className={cn(
                "rounded-full px-2.5 py-1 text-xs font-semibold transition-colors duration-150 motion-reduce:transition-none",
      ...
      {/* Pagination */}
      <div className="border-border/60 mt-3 flex flex-wrap items-center justify-between gap-2 border-t px-4 py-3">
        <p className="text-muted-foreground text-xs">
