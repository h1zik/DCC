...
export function registerSeoTools(server: McpServer, deps: Deps) {
  ...
  server.tool(
    "seo_visibility",
    "Visibility score (0-100%, ala Semrush) + distribusi posisi per proyek rank tracker. Pakai untuk 'seberapa visible kita di Google', 'ringkasan performa SEO'. Dihitung dari data lokal — instan & gratis.",
    {},
    async () => asText(await dccFetch("/api/ai/seo/visibility")),
  );

  server.tool(
    "seo_domain_overview",
    "Potret organik satu domain (analisis tersimpan): estimasi trafik, jumlah keyword per posisi, top keywords, dan kompetitor organik. Tanpa `target` → daftar analisis yang ada. Pakai untuk 'profil SEO domain X', 'trafik organik kompetitor'.",
    {
      target: z
        .string()
        .optional()
    ...
    async ({ target }) =>
      asText(
        await dccFetch(`/api/ai/seo/domain-overview${buildQuery({ target })}`),
      ),
  );
  ...
  server.tool(
    ...
    async ({ gapId, bucket, limit }) =>
      asText(
        await dccFetch(
          `/api/ai/seo/keyword-gap${buildQuery({ gapId, bucket, limit })}`,
        ),
      ),
  );
  ...
  server.tool(
    ...
    "Feed rekomendasi artikel SEO: keyword terbaik untuk digarap (artikel baru vs optimasi konten existing di posisi 5-20), skor peluang, judul usulan, dan stage pipeline (ide→brief→draft→terbit). Pakai untuk 'artikel apa yang sebaiknya ditulis'.",
    {
      stage: z
    ...
    async ({ stage, type, limit }) =>
      asText(
        await dccFetch(
          `/api/ai/seo/content-opportunities${buildQuery({ stage, type, limit })}`,
        ),
      ),
  );
}
